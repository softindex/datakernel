		...
		return client.list("*" + LOG)
				.then(list -> Promises.all(list.stream()
						.filter(file -> {
							...
							assert difference > 0;
							logger.log(Level.FINE, "File {} timestamp {} > {}", new Object[]{file, fileTimestamp, timestamp});
							skipped.inc();
							return false;
						})
						.map(file -> {
							if (logger.isLoggable(Level.FINE)) {
								...
								logger.log(Level.FINE, "Delete file: {} with last modifiedTime: {}({} millis)",
										new Object[]{file.getName(), lastModifiedTime, lastModifiedTime.toMillis()});
							}
							deleted.inc();
