...
public final class RemoteFsClusterClient implements FsClient, Initializable<RemoteFsClusterClient>, EventloopService, EventloopJmxMBeanEx {
	...
	private void markAlive(Object partitionId) {
		...
		if (client != null) {
			logger.log(INFO,"Partition " + partitionId + " is alive again!");
	...
	public boolean markDead(Object partitionId, @Nullable Throwable e) {
		...
		if (client != null) {
			logger.log(WARNING, "marking " + partitionId + " as dead (" + e + ')');
	...
	private Promise<ChannelConsumer<ByteBuf>> upload(String filename, long offset, @Nullable Long revision) {
		...
		return Promises.toList(selected.stream()
				...
				.then(tries -> {
					...
					logger.log(FINEST, () -> "uploading file " + filename + " to "
							+ successes.stream().map(s -> s.id.toString()).collect(joining(", ", "[", "]")) + ", " + this);
				...
				.then(tries -> {
					...
					return Promises.any(found.stream()
							...
							.map(piwfs -> {
								...
								logger.log(FINEST, "downloading file " + name + " from " + piwfs.getValue1());
								return client.download(name, offset, length)
										.whenException(e -> logger.log(WARNING, "Failed to connect to server with key " + piwfs.getValue1() + " to download file " + name, e))
