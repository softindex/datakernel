...
public final class RemoteFsClient implements FsClient, EventloopService {
								...
								.then(msg -> {
									...
									return Promise.of(messaging.sendBinaryStream()
											.withAcknowledgement(ack -> ack
													...
													.whenException(e -> {
														messaging.close(e);
														logger.log(Level.WARNING, "Cancelled while trying to upload file " + filename + " (" + this +"): ");
													})
													.whenComplete(uploadFinishPromise.recordStats())));
								})
								.whenException(e -> {
									messaging.close(e);
									logger.log(Level.WARNING, () -> "Error while trying to upload file " + filename + " (" + e + "): " + this);
								}))
				...
				.then(messaging ->
						messaging.send(new Download(name, offset, length))
								...
								.then(msg -> {
									...
									logger.log(Level.FINEST, "download size for file " + name +" is " + receivingSize + ": " + this);
	...
	private Promise<MessagingWithBinaryStreaming<FsResponse, FsCommand>> connect(InetSocketAddress address) {
		return AsyncTcpSocketImpl.connect(address, 0, socketSettings)
				...
				.whenResult($ -> logger.log(Level.FINEST, () -> "connected to [" + address +"]: " + this))
				.whenException(e -> logger.log(Level.WARNING,() -> "failed connecting to [" + address + "] (" + e + "): " + this))
				.whenComplete(connectPromise.recordStats());
	}

	private <T> Promise<T> handleInvalidResponse(@Nullable FsResponse msg) {
		if (msg == null) {
			logger.log(Level.WARNING, () -> this + ": Received unexpected end of stream");
		...
		return connect(address)
				.then(messaging ->
						messaging.send(command)
								...
								.whenException(e -> {
									messaging.close(e);
									logger.log(Level.WARNING, "Error while processing command " + command +" (" + e +") : " + this);
								}));
	}
