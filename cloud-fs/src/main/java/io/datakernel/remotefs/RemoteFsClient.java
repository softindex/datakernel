...
public final class RemoteFsClient implements FsClient, EventloopService {
		...
		return connect(address)
				.then(messaging ->
						messaging.send(new Upload(filename, offset, revision))
								...
								.whenException(e -> {
									messaging.close(e);
									logger.log(WARNING, () -> "Error while trying to upload file " + filename + " (" + e + "): " + this);
								}))
		...
		return connect(address)
				.then(messaging ->
						messaging.send(new Download(name, offset, length))
									...
									return Promise.of(messaging.receiveBinaryStream()
											...
											.withEndOfStream(eos -> eos
													...
													.then(result -> {
														...
														logger.log(SEVERE, () -> "invalid stream size for file " + name +
								...
								.whenException(e -> {
									messaging.close(e);
									logger.log(WARNING, () -> "error trying to download file " + name + " (offset=" + offset + ", length=" + length + ") (" + e + "): " + this);
								}))
	...
	private <T> Promise<T> handleInvalidResponse(@Nullable FsResponse msg) {
		if (msg == null) {
			logger.log(WARNING, () -> this + ": Received unexpected end of stream");
