window.onload = () => {
  ...
  function updateGraph(pk, repo) {
    fetch('/debug/ot/api/' + pk + '/' + repo)
      .then(r => r.text())
      .then(text => {
        let s = text;//.replace(/label="([^"]*?)\|([^"]*?)"/g, 'label="$1"');
      ...
      .then(svg => {
        ...
        $graph.attr('transform', 'scale(1.5) ' + $graph.attr('transform'));
        ...
        $svg.attr('width', bbox.x + bbox.width + bbox.x);
        $svg.attr('height', bbox.y + bbox.height + bbox.y);
