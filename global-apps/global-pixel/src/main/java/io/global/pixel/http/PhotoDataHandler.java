...
public final class PhotoDataHandler {
	...
	private static final CurrentTimeProvider now = CurrentTimeProvider.ofSystem();
	...
	public static MultipartDataHandler createMultipartHandler(AlbumDao albumDao, Map<String, String> paramsMap, Map<String, Photo> photoMap) {
		return MultipartDataHandler.fieldsToMap(paramsMap, (fieldName, fileName) -> fileName.isEmpty() ?
				...
				albumDao.generatePhotoId()
						.whenResult(id -> photoMap.put(id, Photo.create(EMPTY, now.currentTimeMillis(), fileName)))
