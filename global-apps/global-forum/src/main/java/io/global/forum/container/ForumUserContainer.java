...
import io.global.forum.dao.PostDao;
import io.global.forum.dao.PostDaoImpl;
...
import static io.global.forum.Utils.CHANNEL_OT_SYSTEM;
import static io.global.forum.Utils.COMMENTS_OT_SYSTEM;
...
public final class ForumUserContainer implements UserContainer {
	...
	private final StructuredCodec<MapOperation<Long, Post>> commentOpCodec;
	private final OTStateManager<CommitId, ForumOTOperation> channelStateManager;
	private final Map<String, Promise<OTStateManager<CommitId, MapOperation<Long, Post>>>> commentsStateManagers = new HashMap<>();
	...
	private ForumUserContainer(
			...
			OTStateManager<CommitId, ForumOTOperation> channelStateManager
	) {
	...
	public static ForumUserContainer create(
		...
		MyRepositoryId<ForumOTOperation> videosMyRepositoryId =
				new MyRepositoryId<>(RepoID.of(privKey, videosRepoName), privKey, channelOpCodec);
		OTStateManager<CommitId, ForumOTOperation> videosStateManager =
				createStateManager(eventloop, otDriver, videosMyRepositoryId, CHANNEL_OT_SYSTEM, new ForumState());
		FsClient fsClient = fsDriver.adapt(privKey).subfolder(videosFolder);
		return new ForumUserContainer(eventloop, otDriver, fsClient, privKey.computeKeys(), commentsRepoPrefix, commentOpCodec, videosStateManager);
	}
	...
	public Promise<?> start() {
		ForumState state = (ForumState) channelStateManager.getState();
		return channelStateManager.start()
				...
				.whenResult($ -> state.setListener(op -> op.getMetadataOps().stream()
						...
						.forEach(entry -> {
							...
							SetValue<ThreadMetadata> setValue = entry.getValue();
	...
	@NotNull
	public ForumDao createChannelDao() {
		return new ForumDaoImpl(fsClient, channelStateManager);
	}

	public Promise<@Nullable PostDao> createCommentDao(String videoId) {
		Promise<OTStateManager<CommitId, MapOperation<Long, Post>>> managerPromise = commentsStateManagers.get(videoId);
	...
	private Promise<OTStateManager<CommitId, MapOperation<Long, Post>>> ensureCommentStateManager(String videoId) {
		return Promise.complete()
				.then($ ->
						commentsStateManagers.computeIfAbsent(videoId,
								$2 -> {
									RepoID repoID = RepoID.of(keys, commentsRepoPrefix + "/" + videoId);
									MyRepositoryId<MapOperation<Long, Post>> myRepositoryId = new MyRepositoryId<>(repoID, keys.getPrivKey(), commentOpCodec);
									MapOTState<Long, Post> state = new MapOTState<>(new LinkedHashMap<>());
									OTStateManager<CommitId, MapOperation<Long, Post>> stateManager =
	...
	private Promise<Void> removeCommentStateManager(String videoId) {
		Promise<OTStateManager<CommitId, MapOperation<Long, Post>>> stateManager = commentsStateManagers.remove(videoId);
		return Promise.complete()
