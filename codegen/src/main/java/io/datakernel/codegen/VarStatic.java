...
/**
 * Defines methods which allow to access static fields
 */
public final class VarStatic implements Variable {
	private final Expression owner;
	private final String field;

	VarStatic(Expression owner, String field) {
		this.owner = owner;
		this.field = field;
	}
	...
	public static void setField(Context ctx, Type ownerType, String field, Type valueType) {
		...
		if (ctx.getThisType().equals(ownerType)) {
			Class<?> fieldClass = ctx.getStaticFields().get(field);
			if (fieldClass == null) {
				throw new RuntimeException(format("No static field \"%s\" in generated class %s. %s",
						field,
		...
		throw new RuntimeException(format("No public static field or setter for class %s for field \"%s\". %s ",
	...
	private static Type loadFieldOrGetter(Context ctx, Type ownerType, String field, boolean load) {
		...
		if (ownerType.equals(ctx.getThisType())) {
			Class<?> thisFieldClass = ctx.getStaticFields().get(field);
			if (thisFieldClass != null) {
				...
				throw new RuntimeException(format("No public static field or getter for class %s for field \"%s\". %s",
						ownerType.getClassName(),
						field,
						exceptionInGeneratedClass(ctx)));
			}
