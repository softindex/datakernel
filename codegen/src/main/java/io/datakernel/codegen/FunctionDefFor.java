...
import static io.datakernel.codegen.FunctionDefs.add;
import static io.datakernel.codegen.FunctionDefs.value;
import static io.datakernel.codegen.Utils.newLocal;

public class FunctionDefFor implements FunctionDef {
	private final FunctionDef length;
	private final FunctionDef start;
	private final ForVar forVar;

	FunctionDefFor(FunctionDef length, ForVar forVar) {
		this.length = length;
		this.forVar = forVar;
		this.start = value(0);
	}

	FunctionDefFor(FunctionDef start, FunctionDef length, ForVar forVar) {
		this.length = length;
		this.start = start;
		this.forVar = forVar;
	}
	...
	public boolean equals(Object o) {
		...
		FunctionDefFor that = (FunctionDefFor) o;
