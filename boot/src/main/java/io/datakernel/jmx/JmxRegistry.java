...
public final class JmxRegistry implements JmxRegistryMXBean {
	...
	public void registerSingleton(Key<?> key, Object singletonInstance, @Nullable MBeanSettings settings) {
		...
		if (isJmxMBean(instanceClass)) {
			...
			logger.log(FINEST, () -> format("Instance with key %s was not registered to jmx, " +
		...
		try {
			...
			logger.log(FINEST, () -> format("Instance with key %s was successfully registered to jmx " +
	...
	public void registerWorkers(WorkerPool pool, Key<?> key, List<?> poolInstances, MBeanSettings settings) {
		...
		if (poolInstances.size() == 0) {
			logger.log(INFO, format("Pool of instances with key %s is empty", key.toString()));
			return;
		}

		if (!allInstancesAreOfSameType(poolInstances)) {
			logger.log(INFO, format("Pool of instances with key %s was not registered to jmx because their types differ", key.toString()));
			return;
		}

		if (!isJmxMBean(poolInstances.get(0).getClass())) {
			logger.log(INFO, format("Pool of instances with key %s was not registered to jmx, " +
		...
		try {
			mbs.registerMBean(mbean, objectName);
			logger.log(FINEST, () -> format("Pool of instances with key %s was successfully registered to jmx " +
					"with ObjectName \"%s\"", key.toString(), objectName.toString()));
