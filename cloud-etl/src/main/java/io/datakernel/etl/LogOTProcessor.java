...
public final class LogOTProcessor<T, D> implements EventloopService, EventloopJmxMBeanEx {
	...
	private Promise<LogDiff<D>> doProcessLog() {
		...
		logger.log(Level.FINEST, () -> "processLog_gotPositions called. Positions: " + state.getPositions());
		...
		return supplier.getSupplier().streamTo(consumer.getConsumer())
				...
				.whenResult(logDiff ->
						logger.log(Level.INFO, () -> "Log '" + log + "' processing complete. Positions: " + logDiff.getPositions()));
	}
	...
	private StreamSupplierWithResult<T, Map<String, LogPositionDiff>> getSupplier() {
		...
		for (String partition : partitions) {
			...
			logger.log(Level.INFO, () -> "Starting reading '" + logName +"' from position " + finalLogPosition);
