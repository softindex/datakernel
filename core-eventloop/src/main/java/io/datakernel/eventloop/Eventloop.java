...
public final class Eventloop implements Runnable, EventloopExecutor, Scheduler, Initializable<Eventloop>, EventloopJmxMBeanEx {
	...
	private void openSelector() {
		if (selector == null) {
			try {
				...
				logger.log(SEVERE, "Could not open selector", e);
	...
	private void closeSelector() {
		if (selector != null) {
			try {
				...
				logger.log(SEVERE, "Could not close selector", e);
			}
		}
	}
	...
	public void closeChannel(@Nullable SelectableChannel channel, @Nullable SelectionKey key) {
		...
		try {
			...
			logger.log(WARNING, e, () ->  "Failed to close channel " + channel);
		}
	}
	...
	public void run() {
		...
		while (true) {
			if (!isAlive()) {
				logger.log(INFO, () -> this + " is complete, exiting...");
				break;
			}
			try {
				...
				logger.log(SEVERE, "Selector is closed, exiting...", e);
				break;
			} catch (IOException e) {
		...
		logger.log(INFO, () -> this + " finished");
		eventloopThread = null;
		if (selector != null && selector.isOpen() && selector.keys().stream().anyMatch(SelectionKey::isValid)) {
			logger.log(WARNING, () -> "Selector is still open, because event loop " + this + " has " + selector.keys() + " keys");
			return;
		}
		closeSelector();
	}
		...
		try {
			...
			if (datagramChannel != null) {
				try {
					...
					logger.log(SEVERE, "Failed closing datagram channel after I/O error", nested);
	...
	private void recordIoError(@NotNull Exception e, @Nullable Object context) {
		logger.log(WARNING, () -> "IO Error in " + context + ": " + e.toString());
	}
	...
	public void recordFatalError(@NotNull Throwable e, @Nullable Object context) {
		...
		logger.log(SEVERE, e, () -> "Fatal Error in " + context);
