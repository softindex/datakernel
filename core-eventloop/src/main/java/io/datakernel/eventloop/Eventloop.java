...
public final class Eventloop implements Runnable, EventloopExecutor, Scheduler, Initializable<Eventloop>, EventloopJmxMBeanEx {
	...
	public void run() {
		...
		while (true) {
			if (!isAlive()) {
				logger.log(Level.INFO, () -> this + " is complete, exiting...");
				break;
			}
			try {
		...
		logger.log(Level.INFO, () -> this + " finished");
		eventloopThread = null;
		if (selector != null && selector.isOpen() && selector.keys().stream().anyMatch(SelectionKey::isValid)) {
			logger.log(Level.WARNING, () -> "Selector is still open, because event loop " + this + " has " + selector.keys() + " keys");
			return;
		}
		closeSelector();
	}
	...
	private void recordIoError(@NotNull Exception e, @Nullable Object context) {
		logger.log(Level.WARNING, () -> "IO Error in " + context + ": " + e.toString());
	}
