...
public abstract class BlockingDnsResolver {
	...
	protected final Map<String, DnsFutureResult<InetAddress[]>> pendings = new HashMap<>();
	...
	public static DnsClient getAsDnsClient(final BlockingDnsResolver resolver, final Eventloop eventloop) {
		return new DnsClient() {
			public void resolve(String domainName, final ResultCallback<InetAddress[]> callback, boolean ipv6) {
				...
				final DnsFutureResult<InetAddress[]> future = ipv6 ? resolver.resolve6(domainName) : resolver.resolve4(domainName);
	 ...
	 */
	public DnsFutureResult<InetAddress[]> resolve4(String host) {
	 ...
	 * @return future from which you can get result of resolving
	 */
	public DnsFutureResult<InetAddress[]> resolve6(String host) {
		return resolve(host, true);
	}

	protected DnsFutureResult<InetAddress[]> resolve(String host, boolean ipv6) {
		checkNotNull(host);

		if (HttpUtils.isInetAddress(host)) {
			InetAddress ipAddress = HttpUtils.inetAddress(host);
			return DnsFutureResult.immediateFuture(new InetAddress[]{ipAddress});
		}

		host = host.toLowerCase();
		ResolvedAddress resolvedAddress = cache.get(host);
		if (resolvedAddress != null && !resolvedAddress.isInvalid()) {
			return DnsFutureResult.immediateFuture(resolvedAddress.getAllAddresses());
		}
		return resolvePending(host, ipv6);
	}

	synchronized protected DnsFutureResult<InetAddress[]> resolvePending(final String host, final boolean ipv6) {
		DnsFutureResult<InetAddress[]> pending = pendings.get(host);

		if (pending != null) {
			return pending;
		} else {
			final DnsFutureResult<InetAddress[]> future = DnsFutureResult.create();
