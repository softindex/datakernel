...
import static io.global.comm.util.Utils.formatInstant;
import static java.util.Collections.emptyList;
import static java.util.Map.Entry.comparingByKey;
import static java.util.stream.Collectors.toList;

public class PostView implements Comparable<PostView> {
	...
	public PostView(
			String id, Object content, String initialTimestamp, String lastEditTimestamp, Instant timestamp,
			UserView author,
	...
	private static Promise<Stream<PostView>> inlineChildren(CommDao commDao, String threadId, Post post, @Nullable UserId currentUser, UserRole currentRole, @Nullable PostView parent, PostRenderer renderer) {
		return Promises.toList(post.getChildren().stream()
				.map(p ->
						PostView.from(commDao, threadId, p, currentUser, currentRole, -1, renderer)
								.then(pv -> {
									pv.setInlineParent(parent);
									return inlineChildren(commDao, threadId, p, currentUser, currentRole, pv, renderer).map(s -> Stream.concat(Stream.of(pv), s));
								})))
				.map(ll -> ll.stream().flatMap(Function.identity()).sorted());
	}

	public static Promise<PostView> single(CommDao commDao, String threadId, Post post, @Nullable UserId currentUser, UserRole currentRole, int maxDepth, PostRenderer renderer) {
		Promise<PostView> promise = from(commDao, threadId, post, currentUser, currentRole, -1, renderer);
		if (maxDepth != -1 && post.computeDepth() > maxDepth + 1) { // implies post.getParent() != null
			return Promises.toTuple(promise, single(commDao, threadId, post.getParent(), currentUser, currentRole, -1, renderer))
					.map(t -> {
	...
	public static Promise<PostView> from(CommDao commDao, String threadId, Post post, @Nullable UserId currentUser, UserRole currentRole, int depth, PostRenderer renderer) {
		assert depth >= -1;
		...
		Promise<List<PostView>> childrenPromise =
				depth == -1 ?
						...
						deepest ?
								inlineChildren(commDao, threadId, post, currentUser, currentRole, null, renderer).map(s -> s.collect(toList())) :
								Promises.toList(post.getChildren().stream().sorted(POST_COMPARATOR).map(p -> from(commDao, threadId, p, currentUser, currentRole, depth - 1, renderer)));
