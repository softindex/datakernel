...
public final class DatagraphServer extends AbstractServer<DatagraphServer> {
	...
	private class DownloadCommandHandler implements CommandHandler<DatagraphCommandDownload, DatagraphResponse> {
		...
		public void onCommand(MessagingWithBinaryStreaming<DatagraphCommandDownload, DatagraphResponse> messaging, DatagraphCommandDownload command) {
			...
			if (forwarder != null) {
				logger.log(INFO, () -> "onDownload: transferring " + streamId + ", pending downloads: " + pendingStreams.size());
			} else {
				logger.log(INFO, () -> "onDownload: waiting " + streamId + ", pending downloads: " + pendingStreams.size());
			...
			consumer.withAcknowledgement(ack ->
					ack.whenComplete(($, e) -> {
						if (e != null) {
							logger.log(WARNING, "Exception occurred while trying to send data");
						}
						messaging.close();
					}));
		}
	...
	public <T> StreamConsumer<T> upload(StreamId streamId, Class<T> type) {
		...
		ChannelQueue<ByteBuf> forwarder = pendingStreams.remove(streamId);
		if (forwarder == null) {
			logger.log(INFO, () -> "onUpload: waiting " + streamId + ", pending downloads: " + pendingStreams.size());
			forwarder = new ChannelZeroBuffer<>();
			pendingStreams.put(streamId, forwarder);
		} else {
			logger.log(INFO, () -> "onUpload: transferring " + streamId +", pending downloads: " + pendingStreams.size());
		}
		...
		messaging.receive()
				.whenResult(msg -> {
					if (msg != null) {
						...
						logger.log(WARNING, "unexpected end of stream");
						messaging.close();
					}
				})
				.whenException(e -> {
					logger.log(SEVERE, "received error while trying to read", e);
					messaging.close();
				});
	}
	...
	private void doRead(MessagingWithBinaryStreaming<DatagraphCommand, DatagraphResponse> messaging, DatagraphCommand command) {
		...
		if (handler == null) {
			messaging.close();
			logger.log(SEVERE, () -> "missing handler for " + command);
		} else {
