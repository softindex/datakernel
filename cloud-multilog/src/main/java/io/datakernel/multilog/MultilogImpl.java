	...
	private StreamSupplierWithResult<T, LogPosition> readLogFiles(@NotNull String logPartition, @NotNull LogPosition startPosition, @NotNull List<LogFile> logFiles) {
		...
		Iterator<StreamSupplier<T>> logFileStreams = new Iterator<StreamSupplier<T>>() {
			...
			public StreamSupplier<T> next() {
				...
				logger.log(FINEST, () -> "Read log file `" + currentLogFile + "` from: " + position);
			...
			private void log(Throwable e) {
				if (e == null) {
					logger.log(FINEST, () -> "Finish log file `" + currentLogFile + "` in " + sw +  ", " +
							"compressed bytes: " + inputStreamPosition + " (" + inputStreamPosition / Math.max(sw.elapsed(SECONDS), 1) + " bytes/s)");
				} else {
					logger.log(SEVERE, e, () -> "Error on log file" + currentLogFile
