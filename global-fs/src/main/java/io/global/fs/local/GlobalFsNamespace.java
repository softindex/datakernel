...
public final class GlobalFsNamespace extends AbstractGlobalNamespace<GlobalFsNamespace, GlobalFsNodeImpl, GlobalFsNode> {
	...
	public Promise<ChannelConsumer<DataFrame>> upload(String filename, long offset, long revision) {
		logger.log(Level.FINEST, () -> "uploading to local storage " + filename + ", offset: " + offset);
	...
	public Promise<ChannelSupplier<DataFrame>> download(String fileName, long offset, long length) {
		logger.log(Level.FINEST, () -> "downloading local copy of " + fileName + " at " + space + ", " +
							...
							.then(signedRemoteMeta -> {
								...
								if (localMeta.isTombstone()) {
									if (remoteMeta != null && remoteMeta.isTombstone()) {
										logger.log(Level.FINEST, () -> "both local and remote files " + remoteMeta.getFilename() + " are tombstones");
										return Promise.of(false);
									} else {
										logger.log(Level.INFO, () -> "local file " + localMeta.getFilename() + " is a tombstone, removing remote");
										return into.delete(space, signedLocalMeta)
												.map($ -> true);
									}
								} else {
									if (remoteMeta != null && remoteMeta.isTombstone()) {
										logger.log(Level.INFO, () -> "remote file " + remoteMeta.getFilename() + " is a tombstone, removing local");
										return delete(signedLocalMeta)
												.map($ -> true);
									}
									logger.log(Level.INFO, "pushing local file " + localMeta.getFilename() + " to node " + into);
									return streamDataFrames(node, into, filename, remoteMeta != null ? remoteMeta.getPosition() : 0, localMeta.getRevision())
											.map($ -> true);
								}
							});
				}))
				.whenComplete(toLogger(logger, FINEST, "push", space, into, node));
	}
	...
	public Promise<Boolean> fetch(GlobalFsNode from, String glob) {
		...
		return from.listEntities(space, glob)
									...
									.then(signedLocalMeta -> {
										...
										if (localMeta != null) {
											...
											if (GlobalFsCheckpoint.COMPARATOR.compare(localMeta, remoteMeta) >= 0) {
												logger.log(Level.FINEST, () -> "local file " + localMeta.getFilename() + " is better than remote");
												return Promise.of(false);
											}
											// other file is encrypted with different key
											if (!Objects.equals(localMeta.getSimKeyHash(), remoteMeta.getSimKeyHash())) {
												logger.log(Level.FINEST, () -> "remote file " + remoteMeta.getFilename() + " is encrypted with different key, ignoring");
												return Promise.of(false);
											}
										}
										if (remoteMeta.isTombstone()) {
											logger.log(Level.FINEST, () -> "remote file " + remoteMeta.getFilename() + " is a tombstone with higher revision, removing local");
											return delete(signedRemoteMeta)
													.map($ -> true);
										}
										logger.log(Level.INFO, () -> "remote file " + remoteMeta.getFilename() + " is better than local");
				...
				.whenComplete(toLogger(logger, FINEST, "fetch", space, from, node));
	}
}
