...
public final class DnsCache {
	...
	public DnsQueryCacheResult tryToResolve(DnsQuery query) {
		...
		if (cachedResult == null) {
			logger.log(Level.FINEST, " cache miss");
			return null;
		}
		...
		if (result.isSuccessful()) {
			logger.log(Level.FINEST, () -> query + " cache hit");
		} else {
			logger.log(Level.FINEST, () -> query + " error cache hit");
		}

		if (isExpired(cachedResult)) {
			logger.log(Level.FINEST, () -> query + " hard TTL expired");
			return null;
		} else if (isSoftExpired(cachedResult)) {
			logger.log(Level.FINEST, () -> query + " soft TTL expired");
	...
	public void add(DnsQuery query, DnsResponse response) {
		...
		if (old != null) {
			...
			logger.log(Level.FINEST, () -> "Refreshed cache entry for " + query);
		} else {
			logger.log(Level.FINEST, () -> "Added cache entry for " + query);
		}
	}
	...
	public void performCleanup() {
		...
		while ((peeked = expirations.peek()) != null && peeked.expirationTime <= currentTime) {
			...
			if (response != null) { // if it was not refreshed(so there is a newer response in the queue)
				...
				logger.log(Level.FINEST, () -> "Cache entry expired for " + query);
			}
			expirations.poll();
		}
