...
package io.datakernel.cube_http;
...
import io.datakernel.cube.api.HttpJsonApiServer;
...
public class CubeHttpApiTest {
	...
	public void testHttpJsonApi() throws Exception {
		...
		CubeStructure cubeStructure = CubeTest.cubeStructure(classLoader);
		...
		// serialize query for sending
		CubeQuery query = new CubeQuery()
				.dimensions("key1", "key2")
				.measures("metric1", "metric2", "metric3")
				.eq("key1", 1)
				.eq("key2", 3);
		final String cubeQueryJson = gson.toJson(query);
		logger.info("JSON serialized query: " + cubeQueryJson);
		...
		// create HTTP server
		final AsyncHttpServer httpServer = HttpJsonApiServer.httpServer(cube, eventloop, PORT);
		...
		executor.execute(new Runnable() {
			...
			public void run() {
				...
				parameters.put("query", cubeQueryJson);
