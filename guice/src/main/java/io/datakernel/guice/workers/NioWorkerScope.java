...
public final class NioWorkerScope implements Scope, NioWorkerScopeFactory {
	...
	private Map<Class<?>, List<ServiceGraphModule.KeyInPool>> poolMapClassKey;
		...
		return new Provider<T>() {
			...
			public T get() {
				...
				if (current == null && !scopedObjects.containsKey(key)) {
					...
					if (!poolMapClassKey.containsKey(current.getClass()))
						poolMapClassKey.put(current.getClass(), new ArrayList<ServiceGraphModule.KeyInPool>());
					poolMapClassKey.get(current.getClass()).add(new ServiceGraphModule.KeyInPool(key, currentPool));
				}
				return current;
			}
		};
	...
	public Map<Class<?>, List<ServiceGraphModule.KeyInPool>> getPoolMapClassKey() {
		if (poolMapClassKey == null) {
			return Collections.emptyMap();
		} else {
			return new HashMap<>(poolMapClassKey);
		}
	}
