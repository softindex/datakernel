...
public final class SerializerGenHppcMap implements SerializerGen {
	...
	public FunctionDef serialize(FunctionDef field, SerializerGen serializerGen, final int version, final SerializerStaticCaller serializerCaller) {
		...
		return sequence(length, hppcMapForEach(iteratorType, field,
				new ForVar() {
					@Override
					public FunctionDef forVar(FunctionDef local) {
						return serializerCaller.serialize(keySerializer, local, version);
					}
				},
				new ForVar() {
					@Override
	...
	public FunctionDef deserialize(Class<?> targetType, final int version, final SerializerStaticCaller serializerCaller) {
		...
		final FunctionDef map = let(constructor(hashMapType));
		final Class<?> valueType = valueSerializer.getRawType();
		final Class<?> keyType = keySerializer.getRawType();
		return sequence(length, map, functionFor(length, new ForVar() {
			@Override
			public FunctionDef forVar(FunctionDef local) {
				return sequence(call(map, "put",
								cast(serializerCaller.deserialize(keySerializer, version, keyType), SerializerGenHppcMap.this.keyType),
								cast(serializerCaller.deserialize(valueSerializer, version, valueType), SerializerGenHppcMap.this.valueType)
						), voidFunc()
				);
			}
		}), map);
