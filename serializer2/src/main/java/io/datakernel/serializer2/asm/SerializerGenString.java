...
public class SerializerGenString implements SerializerGen {
	...
	public FunctionDef serialize(FunctionDef field, SerializerGen serializerGen, int version, SerializerStaticCaller serializerCaller) {
		...
		if (maxLength != -1) {
			function = choice(and(cmpGe(maxLen, value(0)), cmpGe(call(cast(field, String.class), "length"), value(maxLength + 1))),
					cast(call(cast(field, String.class), "substring", value(0), maxLen), String.class),
					cast(field, String.class));
		} else {
			function = cast(field, String.class);
		}
